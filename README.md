...
- 門市人員可以填寫客情記錄
  - 客情記錄能記錄客戶的購買金額、性別、類型、國籍、年齡結購、如何得知茶籽堂訊息、備註
...
- 客戶可以現場使用 PAD 自行加入會員
  - 填寫欄位有：姓名、性別、生日、手機號碼、電子信箱、地址、髮質、膚質、傳真 、如何得知茶籽堂
- 門市人員也可以幫客戶創建會員資料
...
- 當客戶結帳時，門市人員可以輸入訂單記錄
  ...
  - 門市人員也可以使用商品列表快速加入購買商品
    - 商品的折扣方式有優惠價、贈品、打折、第二件半價，預設為無
      - 若選擇折扣方式為優惠價，為跳出一彈跳視窗請門市人員自行輸入折扣後價錢
      - 若選擇折扣方式為贈品，則價錢自動變為0元
      - 若選擇折扣方式為打折，為跳出一彈跳視窗請門市人員輸入折扣%
      - 輸入折數百分比為1-100的數字。例如：打9折請輸入90
      - 若選擇折扣方式為第二件半價，那麼買兩件會有一件算半價
      - 買三件也是一件算半價，買四件跟五件則有兩件算半價，以此類推
    - 整筆訂單的折扣方式有使用折價卷、輸入折扣％
      - 若選擇折扣方式為使用折價卷，則輸入折價金額
      - 若選擇折扣方式為輸入折扣％，則輸入折數百分比為1-100的數字。例如：打9折請輸入90
  ...
  - 門市人員可以修改訂單狀態從預購轉為現貨，則訂單狀態可以從未領取變為已領取
  - 這筆訂單記錄，可以註記取貨方式為自取／宅配
    - 如果是宅配的話，門市人員可以點擊發信通知倉庫，即把客戶訂單資料發信箱通知倉庫（已好, 等第三方驗証通知啟用 24 - 48小時）
  - 客戶訂單資料即包含客戶基本資料(會員／路人)和訂單明細
  - 宅配對象可以是會員／路人 
    - 如果是宅配的話，不扣門市庫存數量，商品由倉庫出貨
